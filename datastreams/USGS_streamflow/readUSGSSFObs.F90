...
!every new year read the data, for each station and store it in memory
  ...
  if(LVT_rc%dyr(source).ne.USGSSFobs(source)%yr) then 
     ...
     do i = 1, USGSSFobs(source)%n_stns
        ...
        write(LVT_logunit,*) '[INFO] reading USGS streamflow file ',trim(filename)
              ...
              if(line(1:4).eq."USGS") then 
                 ...
                 if(ios1.eq.0) then 
                    ...
                    if(ios2.eq.0) then 
                       ...
                       t = nint((obstime-USGSSFobs(source)%starttime)/USGSSFobs(source)%timestep)+1
                       if(t.ge.1.and.t.le.366) then 
                          USGSSFobs(source)%q(i,t) = discharge
                       endif
                    endif
                 else
!                 print*, 'problem? ',i,USGSSFobs(source)%stn_name(i)
!                 stop
                 endif
              endif
           end do
  ...
  if((nint(offset)-offset).eq.0) then !only when LIS time matches the observation
     ...
     tind = nint((obstime1-USGSSFobs(source)%starttime)/USGSSFobs(source)%timestep)+1
