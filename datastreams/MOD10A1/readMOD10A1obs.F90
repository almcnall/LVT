...
! !USES:   
  ...
  use LVT_logMod,     only : LVT_logunit, LVT_endrun, LVT_getNextUnitNumber, &
...
!EOP
  ...
  if(mod10a1obs(source)%startflag.or.alarmCheck) then 
     ...
     call create_mod10a1obs_filename(mod10a1obs(source)%odir, LVT_rc%dyr(source), LVT_rc%dmo(source),&
     ...
     if(file_exists) then 
        ...
        open(ftn,file=trim(filename),access='direct',recl=nc*nr)
        read(ftn,rec=1) mod10a1

        do r=1,mod10a1obs(source)%modis_nr
           do c=1,mod10a1obs(source)%modis_nc
              call ij_to_latlon(mod10a1obs(source)%mod_proj,float(c),float(r),&
                   lat, lon)
              r1 = nr-(nint((lat+59.995)/0.01)+1 )
              c1 = (nint((lon+179.995)/0.01)+1)

              snfrac1(c,r) = real(ichar(mod10a1(c1,r1)))
...
!EOP
  ...
  filename = trim(odir)//'/'//trim(fyr)//'.'//trim(fmo)//'.'//trim(fda)//'/'&
       //'Fractional_Snow_Cover.1gd1c'
