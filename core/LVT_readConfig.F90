...
!  call LVT_verify(rc,'Number of processors along y: not defined')
  ...
  call ESMF_ConfigGetAttribute(LVT_config,time, &
       label="Temporal averaging interval:",&
       rc=rc)
  call LVT_verify(rc,'Temporal averaging interval: not defined')
  ...
  call ESMF_ConfigGetAttribute(LVT_config,time, &
       label="Temporal lag in metric computations:",&
       default="0ss", rc=rc)
  call LVT_verify(rc,'Temporal lag in metric computations: not defined')
  ...
  call ESMF_ConfigGetAttribute(LVT_config,time,&
       label="Stats output interval:",&
       rc=rc)
  call LVT_verify(rc,'Stats output interval: not defined')
...
!monthly seasonal mask
  ...
  call ESMF_ConfigGetAttribute(LVT_config,LVT_rc%statsodir,&
       ...
       rc=rc)
  call LVT_verify(rc,'Stats output directory: not defined')
...
! The innovation distribution can be computed only in the DA mode.   
  if(LVT_rc%runmode.eq.LVT_dastatId) then 
     call ESMF_ConfigGetAttribute(LVT_config,LVT_rc%computeInnovDist, &
          label="Compute innovation distribution:", rc=rc)
     call LVT_verify(rc, 'Compute innovation distribution: not defined')
     call ESMF_ConfigGetAttribute(LVT_config,LVT_rc%computeGain, &
          label="Compute analysis gain:", rc=rc)
     call LVT_verify(rc, 'Compute analysis gain: not defined')
