...
!EOP
    ...
    if(LVT_rc%computeInnovDist.eq.1) then 
       allocate(LVT_DAstats%mean_norm_innov(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))
       allocate(LVT_DAstats%var_norm_innov(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))   
       allocate(LVT_DAstats%sx_norm_innov(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))
       allocate(LVT_DAstats%sxx_norm_innov(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))   
       allocate(LVT_DAstats%count_norm_innov(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))
       
       allocate(LVT_DAstats%mean_innov(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))
       allocate(LVT_DAstats%count_mean_innov(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))

       allocate(LVT_DAstats%mean_fvar(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))
       allocate(LVT_DAstats%count_mean_fvar(LVT_LIS_rc(1)%lnc, LVT_LIS_rc(1)%lnr))
    ...
    if(LVT_rc%computeGain.eq.1) then 
       allocate(LVT_DAstats%mean_gain(LVT_LIS_rc(1)%ntiles, LVT_rc%nstvars))   
       allocate(LVT_DAstats%count_mean_gain(LVT_LIS_rc(1)%ntiles, LVT_rc%nstvars))   
...
!EOP
    ...
    character*100 :: innovfile, gainfile
    logical       :: file_exists
    integer       :: ftn
    integer       :: t
