  ...
  if(file_exists) then   
     ...
     if(iret.eq.0) then 
        ...
        call ESMF_TimeSet(currTime,yy=LVT_rc%dyr(source), &
             ...
             rc=status)
        call LVT_verify(status, 'Error in ESMF_TimeSet: readERAinterimLandobs')
        ...
        if(mod(tindex,6).eq.0) then 
           ...
           do r=1,LVT_rc%lnr
              do c=1,LVT_rc%lnc
                 if(sd_in(c,r,1).ne.LVT_rc%udef) then
                    swe(c,r) = sd_in(c,r,1)*rsn_in(c,r,1)
                 endif
              enddo
           enddo
        endif
     ...
     if(iret.eq.0) then 
        ...
        if(mod(tindex,3).eq.0) then 
           ...
           if(tindex.gt.0) then 
              ...
              call interp_ERAIlandvar2d(source,scale_qle_f, offset_qle_v, qle(:,:,1),&
                   qle_out)
              call interp_ERAIlandvar2d(source,scale_qh_f, offset_qh_v, qh(:,:,1),&
                   qh_out)
              call interp_ERAIlandvar2d(source,scale_tp_f, offset_tp_v, tp(:,:,1),&
                   tp_out)
              call interp_ERAIlandvar2d(source,scale_qsm_f, offset_qsm_v, qsm(:,:,1),&
                   qsm_out)
