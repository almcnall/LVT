  ...
  if(LVT_rc%dyr(source).ne.ARSsmobs(source)%yr) then 
     ...
     do k = 1, ARSsmobs(source)%n_stns
        ...
        if(file_exists) then 
           ...
           ios = 0 
           do while(ios.eq.0) 
              read(ftn,*,iostat=ios) yr, doy, hr, mn, sfsm, sfsm_std
...
!BOP
! 
! !ROUTINE: create_ARSsmsmobs_filename
! \label(create_ARSsmsmobs_filename)
!
! !INTERFACE:
subroutine create_ARSsmsmobs_filename(odir, stn, yr, filename)
! 
! !USES:   
  implicit none
!
...
end subroutine create_ARSsmsmobs_filename
...
!EOP
  ...
  filename = trim(odir)//'/'//trim(stn)//'_sm_'//trim(fyr)//'.txt'
