...
!EOP
  ...
  if((scanobs(source)%yr.ne.LVT_rc%dyr(source))) then 
     ...
     do i=1,scanobs(source)%nstns
        call create_SCAN_filename(scanobs(source)%odir, LVT_rc%dyr(source), &
             scanobs(source)%stnid(i), scanfilename)
        
        inquire(file=trim(scanfilename), exist=file_exists) 
        
        if(file_exists) then 
           write(LVT_logunit,*) '[INFO] Reading SCAN file ',trim(scanfilename)
  ...
  t = nint((scantime1 - scanobs(source)%starttime)/scanobs(source)%timestep)+1
  do i=1,scanobs(source)%nstns
