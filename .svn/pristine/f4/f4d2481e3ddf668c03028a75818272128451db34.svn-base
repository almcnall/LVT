...
** Note that if the source file for the module being "use"d cannot be found
** assuming case sensitivity, then the source file is searched for using
** lower case and then using upper case.
**
...
main (int argc, char **argv)
{
  ...
  int case_loop;           /* index for the simple case loop */
  char *cptr;              /* pointer used in simple case loop */
  ...
  */
   ...
   ** A simple loop has been added to search for the srcfile first
   ** assuming case sensitivity, then using lower case, then using upper case.
	*/

	if ( ! already_found (depnam, uselist)) {  /* Skip any duplicates */

	  /*
	  ** Simple case conversion loop
	  */
      for ( case_loop = 0; case_loop < 3; ++case_loop ) {
         if ( case_loop == 0 ) {
            ; /* leave case of depnam as is */
         } else if (case_loop == 1 ) {
            cptr = depnam;
            while ( *cptr ) {
