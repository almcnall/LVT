...
!BOP
! 
! !ROUTINE: readFLUXNETObs
! \label{readFLUXNETObs}
!
! !INTERFACE: 
subroutine readFLUXNETObs(source)
! 
! !USES:   
  use ESMF
  ...
  use LVT_histDataMod
  use FLUXNET_obsMod, only : FLUXNETObs
...
! 
!  NOTES: 
!   The FLUXNET output is available at monthly intervals. So 
...
!EOP
...
#if (defined USE_NETCDF3 || defined USE_NETCDF4)  
  if(FLUXNETobs(Source)%yr.ne.LVT_rc%dyr(source)) then      

     FLUXNETobs(Source)%yr = LVT_rc%dyr(source)

     FLUXNETobs(Source)%qle = LVT_rc%udef
     FLUXNETobs(Source)%qh = LVT_rc%udef
     
     call create_fluxnet_lh_filename(FLUXNETobs(Source)%odir, &
          LVT_rc%dyr(source),filename)
     inquire(file=trim(filename),exist=file_exists) 
     
     if(file_exists) then 
        write(LVT_logunit,*) '[INFO] Reading FLUXNET LH file ',trim(filename)
        ...
        do t=1,12
           ...
           call neighbor_interp(LVT_rc%gridDesc,li,qle1d,&
                lo, FLUXNETobs(Source)%qle(:,t), nc*nr, &
                LVT_rc%lnc*LVT_rc%lnr,&
                FLUXNETobs(Source)%rlat, FLUXNETobs(Source)%rlon, &
                FLUXNETobs(Source)%n11,LVT_rc%udef, ios)
        enddo
     ...
     call create_fluxnet_sh_filename(FLUXNETobs(Source)%odir,&
          LVT_rc%dyr(source),filename)
     
     inquire(file=trim(filename),exist=file_exists) 
     
     if(file_exists) then 
        write(LVT_logunit,*) '[INFO] Reading FLUXNET SH file ',trim(filename)
        ...
        do t=1,12
           ...
           call neighbor_interp(LVT_rc%gridDesc,li,qh1d,&
                lo, FLUXNETobs(Source)%qh(:,t), nc*nr, &
                LVT_rc%lnc*LVT_rc%lnr,&
                FLUXNETobs(Source)%rlat, FLUXNETobs(Source)%rlon, &
                FLUXNETobs(Source)%n11,LVT_rc%udef, ios)
        enddo
  ...
  if(LVT_rc%d_nmo(source).ne.FLUXNETobs(Source)%mo) then 
     cmo = FLUXNETobs(Source)%mo
     write(LVT_logunit,*) '[INFO] Reading FLUXNET data for ',cmo
     do r=1, LVT_rc%lnr
        do c=1, LVT_rc%lnc
           varfield(c,r) = FLUXNETobs(Source)%qle(c+(r-1)*LVT_rc%lnc, &
                cmo)
        enddo
     enddo
     ...
     do r=1, LVT_rc%lnr
        do c=1, LVT_rc%lnc
           varfield1(c,r) = FLUXNETobs(Source)%qh(c+(r-1)*LVT_rc%lnc, &
                cmo)
        enddo
     enddo
     
     FLUXNETobs(Source)%mo = LVT_rc%d_nmo(source)
...
! !DESCRIPTION:
! 
! This routine creates a timestamped filename for FLUXNET_LH data files 
...
!   \item[filename]  Name of the FLUXNET_LH file
! 
! !FILES USED:
!
! !REVISION HISTORY: 
...
! !DESCRIPTION:
! 
! This routine creates a timestamped filename for FLUXNET_SH data files 
! based on the given date (year, month, day)
!
!  The arguments are: 
!  \begin{description}
!   \item[odir]      GlboSnow base directory
!   \item[yr]        year of data
!   \item[filename]  Name of the FLUXNET_SH file
! 
! !FILES USED:
!
! !REVISION HISTORY: 
