...
!EOP
    ...
    if (nldas2data(source)%anlys_data_class.eq."LSM") then
       ...
       if ((LVT_MOC_SOILMOIST(source).ge.1).and.(nldas2data(source)%lsm.eq."SAC")) then
          call create_nldas2data_filename(nldas2data(source)%odir,"SACSM",&
               yr1,mo1,da1,hr1,doy1,sacsmname)
       ...
       do index = 1,nvars
          ...
          if ((pid(index).eq.qs_index).and.(LVT_MOC_QS(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,qs)
          endif
          
          if ((pid(index).eq.qsb_index).and.(LVT_MOC_QSB(source).ge.1)) then
          ...
          if ((pid(index).eq.ecanop_index).and.(LVT_MOC_ECANOP(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,ecanop)
          endif
          
          if ((pid(index).eq.tveg_index).and.(LVT_MOC_TVEG(source).ge.1)) then
             call retrieve_nldas2data(igrib,nc,nr,nvars,index,tveg)
          endif
          
          if ((pid(index).eq.esoil_index).and.(LVT_MOC_ESOIL(source).ge.1)) then
    ...
    call LVT_logSingleDataStreamVar(LVT_MOC_EVAP,source,varfield,vlevel=1,         &
         units="kg/m2s")
    if (LVT_MOC_QS(source).ge.1) then
       do r = 1,nr
          do c = 1,nc
    ...
    call interp_nldas2var(source,nc,nr, qs, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_QS,source,varfield,vlevel=1,           &
         units="kg/m2s")
    if (LVT_MOC_QSB(source).ge.1) then
       do r = 1,nr
          do c = 1,nc
    ...
    call interp_nldas2var(source,nc,nr, qsb, varfield)
    call LVT_logSingleDataStreamVar(LVT_MOC_QSB,source,varfield,vlevel=1,          &
         units="kg/m2s")
    ...
    if (nldas2data(source)%lsm.eq."NOAH") then
       if ((nldas2data(source)%smvol).and.(LVT_MOC_SOILMOIST(source).ge.1)) then
          do r = 1,nr
             do c = 1,nc
...
!
! !INTERFACE:
  subroutine create_nldas2data_filename(odir,lsm,yr,mo,da,hr,doy,  &
       filename)
