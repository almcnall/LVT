       ...
       if(LVT_rc%computeFlag) then 
          ...
          if(linearReg_struc%mode.eq. "single-variable") then 
             if(ds1_inp1%selectNlevs.ge.1.and.ds2%selectNlevs.ge.1) then 
                ...
                do i=1,nsize
                   if(ds1_inp1%count(i,1).ne.0.and.ds2%count(i,1).ne.0) then 
                      do j=1,nsize
                         if(ds1_inp1%count(j,1).ne.0.and.ds2%count(j,1).ne.0) then 
                            data_flag = .true. 
          ...
          elseif(linearReg_struc%mode.eq. "two-variable") then 
             ...
             if(ds1_inp1%selectNlevs.ge.1.and.ds1_inp2%selectNlevs.ge.1&
                ...
                do i=1,nsize
                   if(ds1_inp1%count(i,1).ne.0.and.ds1_inp2%count(i,1).ne.0&
                        .and.ds2%count(i,1).ne.0) then 
                      do j=1,nsize
                         if(ds1_inp1%count(j,1).ne.0.and.ds1_inp2%count(j,1).ne.0&
                              .and.ds2%count(j,1).ne.0) then 
                            data_flag = .true. 
                            if(linearReg_struc%out_of_sample.eq.1) then 
                               if(i.eq.j) then 
                                  data_flag = .false. 
                            ...
                            if(data_flag) then 
                               ...
                               linearReg_struc%sx2x2(j,1) = linearReg_struc%sx2x2(j,1)+ & 
                                    ds1_inp2%value(j,1)*ds1_inp2%value(j,1)
                               
                               linearReg_struc%sx1y(j,1) = linearReg_struc%sx1y(j,1)+ & 
                                    ds1_inp1%value(j,1)*ds2%value(j,1)

                               linearReg_struc%sx2y(j,1) = linearReg_struc%sx2y(j,1)+ & 
                                    ds1_inp2%value(j,1)*ds2%value(j,1)
...
!----------------------------------------------------------------------
             ...
             do i=1,nsize
                if(ds1_inp1%count(i,1).ne.0.and.ds2%count(i,1).ne.0) then                 
                   temp_val = linearReg_struc%a_coef(i,1) + &
                        linearReg_struc%b1_coef(i,1) * ds1_inp1%value(i,1)
                   ds2%value(i,1) = temp_val
                else
                   ds2%value(i,1) = LVT_rc%udef
                endif
             enddo
          endif
       ...
       elseif(linearReg_struc%mode.eq. "two-variable") then        
          ...
          if(ds1_inp1%selectNlevs.ge.1.and.ds1_inp2%selectNlevs.ge.1&
               .and.ds2%selectNlevs.ge.1) then 
             do i=1,nsize
                if(ds1_inp1%count(i,1).ne.0.and.ds1_inp2%count(i,1).ne.0.and.&
                     ds2%count(i,1).ne.0) then                 
                   temp_val = linearReg_struc%a_coef(i,1) + &
                        linearReg_struc%b1_coef(i,1) * ds1_inp1%value(i,1) + & 
                        linearReg_struc%b2_coef(i,1) * ds1_inp2%value(i,1)
                   ds2%value(i,1) = temp_val
                else
                   ds2%value(i,1) = LVT_rc%udef
                endif
